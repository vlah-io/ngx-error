...
const directoryPath = path.join(__dirname, 'projects/ngx-error/src/lib');
const results = [];
...
try {
  ...
  const fp = fs.createWriteStream(path.join(__dirname, 'projects/ngx-error/src/public-api.ts'), {
    flags: 'w'
  });
  results.forEach((str) => {
